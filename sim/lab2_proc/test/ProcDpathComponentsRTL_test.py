...
from lab2_proc.ProcDpathComponentsRTL import ImmGenVRTL

from lab2_proc.ProcDpathComponentsRTL import AluVRTL
...
def test_immgen( test_verilog, dump_vcd ):
  ...
  run_test_vector_sim( ImmGenVRTL(), [ header_str,
    # imm_type inst                                imm
...
def test_alu_sub( dump_vcd, test_verilog ):
  run_test_vector_sim( AluVRTL(), [
    ...
    [ 0x7fffffff,   0x00000000,   1,  0x7fffffff,   '?',      '?',       '?'      ],
    #pos-neg
    [ 0x7fffffff,   0x00000001,   1,  0x7ffffffe,   '?',      '?',       '?'      ],
    [ 0x00000000,   0xffffffff,   0,  0xffffffff,   '?',      '?',       '?'      ],
    # neg-neg
